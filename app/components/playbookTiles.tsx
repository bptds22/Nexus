/* ═══════════════════════════════════════════════════════════════
   Tuiles du fond « playbook » — X, O, flèches et schémas tactiques, la
   signature visuelle de Nexus. Module SANS 'use client' : il sert à la fois
   au fond ANIMÉ de la page d'accueil (app/components/PlaybookBackground.tsx,
   composant client) et au fond STATIQUE de /app (PlaybookStatique, rendu
   par le serveur, aucun JavaScript).

   Chaque tuile couvre tout le cadre 1440×900. Les schémas y sont DESSINÉS
   une seule fois : l'accueil et /app ne peuvent pas diverger.

   POINTES DE FLÈCHE — identifiants PARAMÉTRÉS. Un <marker> SVG est désigné
   par un id GLOBAL au document : deux SVG qui déclarent chacun « ah »
   entrent en collision, et le navigateur prend le premier trouvé. Chaque
   appelant passe donc ses propres identifiants (voir marqueursPlaybook) :
   l'accueil garde « ah » / « ah2 » à l'identique, /app les préfixe.
   ═══════════════════════════════════════════════════════════════ */

/** Identifiants des deux pointes de flèche, sous forme d'URL de référence. */
export type MarqueursPlaybook = { ah: string; ah2: string };

/** Les identifiants à déclarer (`ids`) et les références à poser sur les
 *  tracés (`refs`), à partir d'un préfixe. Préfixe vide → « ah » / « ah2 »,
 *  exactement les identifiants historiques de la page d'accueil. */
export function marqueursPlaybook(prefixe = "") {
  const ids = { ah: `${prefixe}ah`, ah2: `${prefixe}ah2` };
  const refs: MarqueursPlaybook = { ah: `url(#${ids.ah})`, ah2: `url(#${ids.ah2})` };
  return { ids, refs };
}

/** Les deux <marker>, à placer dans un <defs> d'UN SEUL des SVG de la page :
 *  les identifiants sont globaux, les autres SVG les retrouvent. */
export function DefsPlaybook({ ids }: { ids: { ah: string; ah2: string } }) {
  return (
    <defs>
      <marker id={ids.ah}  markerWidth="10" markerHeight="8" refX="10" refY="4" orient="auto">
        <polygon points="0 0, 10 4, 0 8" fill="#7B94B0"/>
      </marker>
      <marker id={ids.ah2} markerWidth="8"  markerHeight="6" refX="8"  refY="3" orient="auto">
...
const tile0 = (m: MarqueursPlaybook) => (
  <>
    <g transform="rotate(-4, 230, 390)" stroke="#7B94B0" strokeWidth="2.2" fill="none">
      ...
      <path d="M18,370 C16,294 -8,246 -26,268 C-44,290 -35,336 -10,342" strokeWidth="2.5" markerEnd={m.ah}/>
      <path d="M370,370 C368,310 320,262 240,248" strokeWidth="2.5" markerEnd={m.ah}/>
      <path d="M190,598 C228,630 310,640 374,632" strokeDasharray="10 7" markerEnd={m.ah2}/>
      <line x1="190" y1="488" x2="190" y2="436" strokeWidth="2.5" markerEnd={m.ah2}/>
    </g>
    <g transform="rotate(3, 1060, 360)" stroke="#7B94B0" strokeWidth="2.2" fill="none">
      ...
      <path d="M1068,481 C1096,434 1144,356 1128,210 C1120,156 1078,106 1062,97" strokeWidth="2.6" markerEnd={m.ah}/>
      <path d="M1148,286 C1134,228 1104,152 1068,100" strokeWidth="2.6" markerEnd={m.ah}/>
      <path d="M1320,448 C1288,408 1234,358 1180,316" strokeDasharray="10 7" markerEnd={m.ah2}/>
      <path d="M1042,484 C984,466 900,462 820,462" strokeDasharray="14 8" markerEnd={m.ah2}/>
    </g>
    <g transform="rotate(-2, 720, 760)" stroke="#7B94B0" strokeWidth="2.2" fill="none">
      ...
      <path d="M720,676 C720,664 720,654 720,648" strokeWidth="3" markerEnd={m.ah}/>
      <path d="M590,834 C720,822 848,830 850,834" strokeDasharray="12 7" markerEnd={m.ah2}/>
      <path d="M870,814 C840,770 790,720 730,658" strokeWidth="2.6" markerEnd={m.ah}/>
      <path d="M462,702 C500,676 580,660 680,650" strokeWidth="2.4" markerEnd={m.ah2}/>
      <path d="M978,702 C940,676 860,660 760,652" strokeDasharray="10 7" markerEnd={m.ah2}/>
      <path d="M570,814 C562,784 556,758 550,736" strokeWidth="2.2" markerEnd={m.ah2}/>
    </g>
    {/* ── Handwritten text ── */}
...
const tile1 = (m: MarqueursPlaybook) => (
  <>
    ...
    <g transform="rotate(-3, 300, 650)" stroke="#7B94B0" strokeWidth="2.2" fill="none">
      ...
      {/* Go routes outside */}
      <path d="M60,600 C58,530 54,466 50,420" strokeWidth="2.5" markerEnd={m.ah}/>
      <path d="M560,600 C562,530 566,466 570,420" strokeWidth="2.5" markerEnd={m.ah}/>
      {/* Crossing / in-route inner WR */}
      <path d="M130,600 C160,566 220,548 290,544" strokeDasharray="10 7" markerEnd={m.ah2}/>
      <path d="M490,600 C460,566 400,548 330,544" strokeDasharray="10 7" markerEnd={m.ah2}/>
      {/* RB screen */}
      <path d="M308,760 C348,778 400,786 450,782" strokeDasharray="8 6" markerEnd={m.ah2}/>
    </g>
    {/* ── Handwritten text ── */}
    ...
    <g transform="rotate(2, 720, 220)" stroke="#7B94B0" strokeWidth="2.2" fill="none">
      ...
      {/* PG drive */}
      <path d="M720,200 C720,160 720,110 720,60" strokeWidth="2.6" markerEnd={m.ah}/>
      {/* Left wing fills lane */}
      <path d="M560,140 C590,100 650,62 706,52" strokeWidth="2.4" markerEnd={m.ah}/>
      {/* Right wing fill */}
      <path d="M880,140 C850,100 790,62 734,52" strokeDasharray="10 7" markerEnd={m.ah2}/>
      {/* Trailer space */}
      <path d="M720,310 C680,280 630,240 590,200" strokeDasharray="12 7" markerEnd={m.ah2}/>
    </g>
    ...
    <g transform="rotate(2, 1200, 260)" stroke="#7B94B0" strokeWidth="2.2" fill="none">
      ...
      {/* C → LD first pass */}
      <path d="M1182,256 C1160,216 1130,178 1118,158" strokeWidth="2.4" markerEnd={m.ah}/>
      {/* LD breaks up left wall */}
      <path d="M1100,130 C1092,96 1086,68 1080,46" strokeWidth="2.4" markerEnd={m.ah}/>
      {/* RD option outlet */}
      <path d="M1300,130 C1308,96 1314,68 1320,46" strokeDasharray="10 7" markerEnd={m.ah2}/>
      {/* C → RD alternative */}
      <path d="M1218,258 C1248,218 1274,178 1286,158" strokeDasharray="8 6" markerEnd={m.ah2}/>
      {/* RW swings through middle */}
      <path d="M1340,340 C1306,306 1256,280 1220,270" strokeDasharray="10 7" markerEnd={m.ah2}/>
    </g>
  </>
);

/* TILE 2 — RPO / Zone Read (upper-left) · 3-on-2 Rush (center) · Motion Offense (bottom-right) */
const tile2 = (m: MarqueursPlaybook) => (
  <>
    ...
    <g transform="rotate(-2, 360, 190)" stroke="#7B94B0" strokeWidth="2.2" fill="none">
      ...
      <path d="M368,272 C344,244 312,218 292,200" strokeWidth="2.6" markerEnd={m.ah}/>
      {/* WR left motion (dashed pre-snap) */}
      <circle cx="100" cy="190" r="20"/>
      <path d="M120,190 C160,188 200,186 248,186" strokeDasharray="8 6" markerEnd={m.ah2}/>
      {/* WR right deep post */}
      <circle cx="640" cy="190" r="20"/>
      <path d="M638,170 C624,114 598,64 566,40" strokeWidth="2.5" markerEnd={m.ah}/>
      {/* TE over middle */}
      <circle cx="530" cy="190" r="20"/>
      <path d="M526,170 C514,130 494,94 460,76" strokeDasharray="10 7" markerEnd={m.ah2}/>
    </g>
    ...
    <g transform="rotate(3, 720, 460)" stroke="#7B94B0" strokeWidth="2.2" fill="none">
      ...
      {/* C drives slot */}
      <path d="M720,520 C720,490 720,450 720,378" strokeWidth="3" markerEnd={m.ah}/>
      {/* Left wing cuts to back door */}
      <path d="M590,520 C614,482 658,446 702,386" strokeWidth="2.4" markerEnd={m.ah}/>
      {/* Right wing delayed */}
      <path d="M850,520 C826,484 782,450 742,384" strokeDasharray="10 7" markerEnd={m.ah2}/>
      {/* Cross-ice pass option */}
      <path d="M704,534 C660,532 618,530 606,530" strokeDasharray="8 6" markerEnd={m.ah2}/>
    </g>
    ...
    <g transform="rotate(-3, 1100, 710)" stroke="#7B94B0" strokeWidth="2.2" fill="none">
      ...
      {/* Ball to left wing */}
      <path d="M1082,648 C1044,630 986,620 948,618" strokeWidth="2.5" markerEnd={m.ah}/>
      {/* Left wing dribble drive baseline */}
      <path d="M930,596 C944,562 984,548 1086,546" strokeWidth="2.5" markerEnd={m.ah}/>
      {/* Right wing back-door cut */}
      <path d="M1270,596 C1240,558 1180,546 1114,546" strokeDasharray="10 7" markerEnd={m.ah2}/>
      {/* Corner skip pass */}
      <path d="M930,618 C958,692 996,762 1060,800" strokeDasharray="12 8" markerEnd={m.ah2}/>
      {/* Weak corner pop */}
      <path d="M1280,778 C1290,738 1296,690 1296,650" strokeDasharray="8 6" markerEnd={m.ah2}/>
    </g>
    {/* ── Handwritten text ── */}
...
const tile3 = (m: MarqueursPlaybook) => (
  <>
    ...
    <g transform="rotate(4, 1050, 380)" stroke="#7B94B0" strokeWidth="2.2" fill="none">
      ...
      <path d="M1058,428 C1100,446 1162,450 1218,436" strokeWidth="2.5" markerEnd={m.ah}/>
      {/* RB fake inside (dashed) */}
      <circle cx="1058" cy="476" r="20"/>
      <path d="M1050,458 C1034,426 1014,396 998,376" strokeDasharray="8 6" markerEnd={m.ah2}/>
      {/* WR left over the top */}
      <circle cx="790" cy="360" r="20"/>
      <path d="M788,340 C780,286 776,240 782,200" strokeWidth="2.5" markerEnd={m.ah}/>
      {/* WR right flag corner */}
      <circle cx="1330" cy="360" r="20"/>
      <path d="M1332,340 C1340,290 1360,252 1392,224" strokeWidth="2.5" markerEnd={m.ah}/>
      {/* TE seam over middle */}
      <circle cx="1220" cy="360" r="20"/>
      <path d="M1218,340 C1208,300 1196,264 1180,234" strokeDasharray="10 7" markerEnd={m.ah2}/>
    </g>
    ...
    <g transform="rotate(-2, 360, 710)" stroke="#7B94B0" strokeWidth="2.2" fill="none">
      ...
      {/* PG hits left elbow */}
      <path d="M344,698 C316,672 272,658 258,654" strokeWidth="2.5" markerEnd={m.ah}/>
      {/* Right high pops */}
      <path d="M480,630 C440,612 380,600 330,598" strokeDasharray="10 7" markerEnd={m.ah2}/>
      {/* Right low baseline cut */}
      <path d="M480,748 C418,748 348,752 258,770" strokeWidth="2.4" markerEnd={m.ah}/>
      {/* Left low seal and dive */}
      <path d="M240,788 C256,738 286,676 302,616" strokeDasharray="8 6" markerEnd={m.ah2}/>
    </g>
    ...
    <g transform="rotate(-3, 1160, 730)" stroke="#7B94B0" strokeWidth="2.2" fill="none">
      ...
      <path d="M1142,840 C1106,810 1060,778 1044,762" strokeWidth="2.4" markerEnd={m.ah}/>
      {/* Left shot on net */}
      <path d="M990,736 C1012,698 1054,646 1104,610" strokeWidth="2.6" markerEnd={m.ah}/>
      {/* Right outlet option */}
      <path d="M1178,842 C1220,810 1264,782 1314,766" strokeDasharray="10 7" markerEnd={m.ah2}/>
      {/* PK breakout attempt */}
      <path d="M1060,646 C1062,618 1094,590 1133,582" strokeDasharray="8 6" markerEnd={m.ah2}/>
    </g>
    {/* ── Handwritten text ── */}
...
/** Les quatre tuiles, dans l'ordre d'empilement de la page d'accueil. */
export const TUILES_PLAYBOOK = [tile0, tile1, tile2, tile3];
