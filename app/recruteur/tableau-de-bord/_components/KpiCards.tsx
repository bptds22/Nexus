...
import { getPipelineCounts } from "../../_data/mockPipelineData";
...
export default function KpiCards({ data }: { data: RecruiterKpiData }) {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-[#1A1D24] rounded-xl border border-[#2D3748] p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/recruteur/favoris" className="text-[12px] font-bold text-[#E63946] hover:text-[#60A5FA] transition-colors flex items-center gap-1">
            Voir tout
        ...
        <div className="grid grid-cols-3 lg:grid-cols-6 gap-3">
          {PIPELINE_STATUSES.map((status, i) => {
            ...
            return (
              <Link
                key={status}
                href={`/recruteur/favoris?filtre=${status}`}
                className="relative group rounded-lg border p-3 text-center transition-all hover:-translate-y-0.5"
                style={{
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        {(() => {
          ...
          const contacted = counts.contacte + counts.en_discussion + counts.visite_planifiee + counts.engage + counts.lettre_signee;
          const committed = counts.engage + counts.lettre_signee;
          const rate = contacted > 0 ? Math.round((committed / contacted) * 100) : 0;
          return (
            <div className="group bg-[#1A1D24] rounded-xl border border-[#2D3748] hover:border-[#E63946]/20 px-5 py-4 flex items-center gap-4 relative overflow-hidden transition-all duration-300">
              <div className="absolute top-0 left-0 right-0 h-[2px] bg-[#E63946] scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left" />
              <div className="w-10 h-10 rounded-full bg-[#E63946]/15 flex items-center justify-center shrink-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-[#6b7280]">{committed}/{contacted}</span>
              </div>
            </div>
          );
        })()}
