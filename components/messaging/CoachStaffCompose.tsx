    ...
    (async () => {
      ...
      if (schoolId) {
        const { data: aths } = await supabase
          .from("athletes")
          .select("id, first_name, last_name, photo_url, cote_globale_entraineur, sports!sport_id(nom), positions!position_id(abreviation, nom)")
        ...
        athOpts = (aths ?? []).map((a) => {
          ...
          const sptRaw = (a as { sports?: unknown }).sports;
          const spt = (Array.isArray(sptRaw) ? sptRaw[0] : sptRaw) as { nom?: string } | null;
          ...
          return {
            ...
            stars: Number((a as { cote_globale_entraineur?: number }).cote_globale_entraineur) || 0,
          };
        });
      }
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
              {athletes.map((a) => (
                <AthleteSelectCard
                  ...
                  position={a.position}
                  stars={a.stars}
                  selected={athleteId === a.id}
                  disabled={!!busyId}
                  onClick={() => setAthleteId(athleteId === a.id ? null : a.id)}
                />
              ))}
            </div>
          )
