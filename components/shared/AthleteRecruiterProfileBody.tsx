...
import {
  sectionLabel, cardBase, pillBase, FreeLock, InfoRow,
  SectionFaitsSaillants, SectionParcoursEquipes, SectionProfilAcademique, SectionInfosPersonnelles,
  SectionMesures, SectionTests, SectionInfosSportives, SectionDetailsAcademiques, SectionMediasLiens,
} from "@/components/shared/athlete/sectionsFiche";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { createClient } from "@/lib/supabase/client";
import { displayFullName, LOCKED_NAME_LABEL } from "@/lib/queries/shared/recruiterAthleteCards";
...
export default function AthleteRecruiterProfileBody({ athleteId, viewerMode }: AthleteRecruiterProfileBodyProps) {
  ...
  /* Données de la fiche — chargement partagé avec le panneau latéral du
     pipeline (lot C0) : components/shared/athlete/useFicheAthleteDonnees.ts. */
  const {
    a, setA, loadingAthlete, grilleSrc, recruitmentStatus, committedSchoolName,
    openToOffers, coachId, affiliation, teamDetails, athleteUserId,
  } = useFicheAthleteDonnees({ id, viewerMode, tierLoading, isFreeRecruiter });
  ...
  useEffect(() => { setA(prev => (prev ? { ...prev, favoriteCount: favCount } : prev)); }, [favCount, setA]);
  ...
  return (
    <div className="min-h-screen relative z-1">
      ...
      <div className={`max-w-7xl mx-auto px-6 py-8 space-y-6 relative z-1 ${contactable ? "pb-28" : "pb-40"}`}>
        ...
        <SectionFaitsSaillants a={a} verrouille={lockContent} />

        <SectionParcoursEquipes a={a} verrouille={lockContent} />

        <SectionProfilAcademique a={a} verrouille={lockContent} estPartenaire={isPartner} />
        ...
        {isDetailed && (
          <div className="space-y-6" key="detailed-sections">
            ...
            <SectionInfosSportives a={a} estPartenaire={isPartner} teamDetails={teamDetails} />

            <SectionDetailsAcademiques a={a} estPartenaire={isPartner} />

            <SectionMediasLiens a={a} verrouille={lockContent} />
