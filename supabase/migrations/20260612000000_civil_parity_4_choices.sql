-- ═══════════════════════════════════════════════════════════════
-- iter civil-parity-rpc — Parité complète école : 4 director choices
-- pour finish_coach_civil_onboarding.
--
-- Contexte
-- --------
-- La version 20260611100000 acceptait UNIQUEMENT 'owner' | 'invite'
-- côté civil (calque du wizard web civil qui montrait 2 cartes). Cette
-- migration ouvre les 4 choix (parité école) :
--   owner    → admin_claims DIRECTEUR PENDING, RPRP requis
--   interim  → admin_claims INTERIM   PENDING, RPRP requis
--   invite   → pending_director_invite stocké, pas de claim
--   coach_only → ni claim ni invite, RPRP non requis
-- TOUS posent school_coaches.role='COACH'. is_school_admin jamais touché.
--
-- Le UI mobile civil actuel (CoachOnboardingMobileCivil) n'expose que
-- 2 cartes ; la RPC accepte désormais les 4 pour ne pas être le
-- bottleneck si une UI future (mobile civil v2, web civil 2c) veut
-- offrir interim/coach_only. La RPC reste la source de vérité.
--
-- Verbatim école (cf. finish_coach_school_onboarding 20260610110000) :
--   - Validation des 4 choix.
--   - Gate responsable identique : NOT has_resp AND choice NOT IN
--     ('owner','interim') → SCHOOL_REQUIRES_RESPONSABLE.
--   - Gate RPRP identique : IN ('owner','interim') AND NOT accepted →
--     RPRP_REQUIRED.
--   - admin_type / rprp_accepted_at / admin_claims claim_type mappés
--     verbatim.
--
-- Écarts (assumés) :
--   - pending_director_invite.type = 'league' (civil) vs 'school' (école)
--     — discriminateur préservé pour un futur admin tab civil.
--   - Pas d'INVALID_SPORT côté école (école n'a pas de création d'équipe
--     en RPC ; civil oui).
--   - Création du club préservée (cas civil-specific).
--
-- ⚠️ Nouveau club créé → has_responsable = false (le club venant d'être
-- créé n'a aucun admin_claims). Donc seuls owner ou interim acceptés
-- à la création — invite + coach_only sont rejetés.
-- (Cohérent : on ne crée pas un club orphelin sur Nexus.)
-- ═══════════════════════════════════════════════════════════════
...
-- GRANT identique au précédent (signature inchangée).
