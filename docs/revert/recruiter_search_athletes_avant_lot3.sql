-- SAUVEGARDE DE REVERT — recruiter_search_athletes AVANT le Lot 3
-- Relevee en prod le 2026-09-07, juste avant apply_migration 20260908015840.
--
-- AUTO-VERIFIEE : md5 du corps normalise = 74dc1d19c2cfc057d44d9f63d079adc1
-- Empreinte relevee par pg_get_functiondef avant l'apply = 74dc1d19c2cfc057d44d9f63d079adc1
-- >>> IDENTIQUE — cette sauvegarde EST la fonction d avant.
--
-- 34 colonnes projetees. ACL d'origine : {postgres, authenticated, service_role} (PUBLIC et anon REVOQUES).
-- Pour revenir en arriere : DROP FUNCTION (meme signature), puis ce CREATE,
-- puis revoke PUBLIC + revoke anon + grant authenticated, service_role,
-- puis reposer le COMMENT d'origine (sans la phrase « LOT 3 : ... »).
